	...
	public static void main(String[] args) {
		...
		try {
			...
			while (rs.next()) {
				...
				pstmt = conn.prepareStatement("SELECT tc.id as id, tc.field_name as field_name, tc.field_type as field_type, tc.read_element as read_element, tc.input_value as input_value, tc.string as string, tc.action as action, ts.base_url as base_url, tc.action_url as action_url FROM test_cases tc, case_suites cs, test_suites ts WHERE cs.test_case_id = tc.id AND ts.id = cs.test_suite_id AND cs.test_suite_id = ? ORDER BY cs.sequence");
				...
				if (pstmt.execute() == true) {
					...
					while (rsForTestCases.next()) {
						...
						if (identifyTestCase(rsForTestCases.getString("field_type"), rsForTestCases.getString("input_value"), rsForTestCases.getString("action")) == "INSPECT") {
							...
							try {
							status = chromeTester.actionPageElement(app.getUrl(), app.getLoginName(), app.getLoginPwd(), rsForTestCases.getString("field_name"), rsForTestCases.getString("field_type"), rsForTestCases.getString("read_element"), rsForTestCases.getString("base_url"));
							}catch (NoSuchElementException nse) {
								logger.error(nse.getMessage());
								updateTestCaseWithError(conn, currentTestCaseId, currentSchedulerId);
								//nse.printStackTrace();
